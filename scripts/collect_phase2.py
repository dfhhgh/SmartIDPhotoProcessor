"""
Phase 2: Unique Dataset Replacement Collection for parser_finetune_current.
Uses the fixed dual-index dedup system from dataset_builder/dedup.py.
"""
import hashlib
import json
import logging
...
DEDUP_INDEX_DIR = CURRENT_DIR / "dedup_index_phase2"
BASELINES_PATH = CURRENT_DIR / "dedup_baselines_phase2.json"
...
def verify_baselines():
    ...
    if violations:
        ...
        for v in violations:
            log.error("  %s", v)
        return False
    log.info("Baseline verification: PASS")
    return True
...
def main():
    ...
    log.info("Building dedup index...")
    dedup = DedupIndex(index_dir=DEDUP_INDEX_DIR, hash_size=16, threshold=5)
    n_aligned = dedup.load_aligned_from_directory(IMAGES_DIR)
    log.info("Indexed %d existing aligned images", n_aligned)
    n_raw = dedup.load_raw_from_directory(RAW_DIR)
    log.info("Indexed %d raw dataset images", n_raw)
    log.info("RAW index: %d, ALIGNED index: %d", dedup.raw_count, dedup.aligned_count)
    ...
    current_cats = {}
    for img_stem in existing_ids:
        meta_path = METADATA_DIR / (img_stem + ".json")
        if meta_path.exists():
    ...
    total_current = len(existing_ids)
    ...
    for cat, target in targets.items():
        ...
        if cur >= target:
            log.info("=== %s: SKIP (current %d >= target %d) ===", cat, cur, target)
            continue
        log.info("=== %s (target: %d, current: %d) ===", cat, target, cur)
        cat_accepted = 0
            ...
            for source in sources:
                ...
                while cat_accepted < target and page <= max_pages:
                    ...
                    for result in results:
                        ...
                        if dedup.is_raw_duplicate(local_path):
                            dup_rejected += 1
                        ...
                        try:
                            ...
                            log.warning("Alignment error for %s: %s", sample_id, e)
                            local_path.unlink(missing_ok=True)
                            continue

                        if dedup.is_aligned_duplicate(aligned_path):
                            aligned_dup_rejected += 1
                        ...
                        dedup.add_raw(sample_id, local_path)
                        dedup.add_aligned(sample_id, aligned_path)
                        local_path.unlink(missing_ok=True)
    ...
    report = {
        ...
        "aligned_dup_rejected": aligned_dup_rejected,
        "per_source": per_source,
        "per_category": per_category,
        "final_total": len(list(IMAGES_DIR.glob("*.png"))),
        "raw_index_size": dedup.raw_count,
        "aligned_index_size": dedup.aligned_count,
    }
    REPORTS_DIR.mkdir(parents=True, exist_ok=True)
    (REPORTS_DIR / "phase2_collection_report.json").write_text(
        json.dumps(report, indent=2), encoding="utf-8"
    )

    violations = verify_baselines()
    if not violations:
        log.error("POST-COLLECTION VERIFICATION FAILED")
        sys.exit(1)

    log.info("Collection complete: %d accepted, %d downloaded, %d raw-dup, %d face-rejected, %d quality-rejected, %d aligned-dup",
             accepted, downloaded, dup_rejected, face_rejected, quality_rejected, aligned_dup_rejected)
