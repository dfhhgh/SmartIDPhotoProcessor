...
from models.export_result import ExportResult, ExportQuality
from models.validation_type import ValidationType
from pipeline.photo_validation_pipeline import PhotoValidationPipeline


def test_pipeline_attaches_export_result_when_validation_succeeds(valid_image):
    """A valid run should attach an export result without replacing the raw crop."""
    ...
    mock_crop_result = CropResult(image=cropped_image, crop_x=10, crop_y=20)
    ...
    mock_exporter.export.return_value = export_result
    ...
    assert result.cropped_image is cropped_image
    assert result.export_result is export_result
    mock_exporter.export.assert_called_once_with(cropped_image)


def test_pipeline_skips_export_for_invalid_validation(valid_image):
    """Invalid validation should not produce an export result."""
    mock_detector = MagicMock()
    ...
    mock_cropper = MagicMock()
    mock_cropper.crop.return_value = CropResult(image=np.zeros((100, 100, 3), dtype=np.uint8), crop_x=1, crop_y=2)

    mock_transformer = MagicMock()
    mock_transformer.transform.return_value = Face()
    ...
    mock_orchestrator = MagicMock()
    mock_orchestrator.validate.return_value = invalid_validation_result
